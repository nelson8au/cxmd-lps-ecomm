...
;(function ($) {
                ...
                if (strs[1]) {
                    if(strs[1]=='*'){
                        if (str.length < strs[0]) {//1,*（设置最小值）
                            var res = {status: 0, info: '字数不能少于'+strs[0]+'个'};
                            return res;
                        }
                    }else if (strs[1] < str.length || str.length < strs[0]) {
                            var res = {status: 0, info: '要求字数为'+strs[0]+'-'+strs[1]+'个'};
                            return res;
                    }
                }else {
    ...
    var checkEmail = function () {
        ...
        if(patterns['Email'].test(str)){
            ...
            var res = {status: 0, info: '请输入正确Email地址！'}
        }
        return res;
    }
    ...
    var showInfo = function (str,status) {
            ...
            if(status == 1 ||  status == true){
                check_form.success(str);
                check_form.remove();//如果正确就移除提示消息
            }
            else{
    ...
    var check_form = {
        ...
        error: function (str) {
            ...
            html = '<div class="send '+color+'">'+icon+' '+ str + '</div>';
            obj.parent().find('.show_info').html(html);
        },
