...
class AdminTreeListBuilder extends AdminBuilder
{
    ...
    public function buttonNew($href, $title = 'Add', $attr = array())
    {
        $attr['href'] = $href;
    ...
    public function buttonDisable($url = null, $title = 'Disable', $attr = array())
    {
        if (!$url) $url = $this->_setStatusUrl;
        return $this->buttonSetStatus($url, 0, $title, $attr);
    }

    public function buttonEnable($url = null, $title = 'Enable', $attr = array())
    {
        if (!$url) $url = $this->_setStatusUrl;
     ...
     * 删除到回收站
     */
    public function buttonDelete($url = null, $title = 'Delete', $attr = array())
    {
        if (!$url) $url = $this->_setStatusUrl;
        return $this->buttonSetStatus($url, -1, $title, $attr);
    }

    public function buttonRestore($url = null, $title = 'Restore', $attr = array())
    {
        if (!$url) $url = $this->_setStatusUrl;
        return $this->buttonSetStatus($url, 1, $title, $attr);
    }

    public function buttonSort($href, $title = 'Sort', $attr = array())
    {
        $attr['href'] = $href;
        return $this->button($title, $attr);
    }
    ...
    {
        $map = array(-1 => 'Delete', 0 => 'Disable', 1 => 'Enable', -2 => 'Not Reviewed');
    ...
    public function keyTitle($name = 'title', $title = 'Title')
    {
        return $this->keyText($name, $title);
    }

    public function keyDoAction($getUrl, $text, $title = 'Action')
    {
        //获取默认getUrl函数
        if (is_string($getUrl)) {
            $getUrl = $this->createDefaultGetUrlFunction($getUrl);
    ...
    public function keyDoActionEdit($getUrl, $text = 'Edit')
    {
        return $this->keyDoAction($getUrl, $text);
    }

    public function keyDoActionRestore($text = 'Restore')
    {
        $that = $this;
    ...
    public function display($templateFile = '', $charset = '', $contentType = '', $content = '', $prefix = '')
    {
        ...
        View::assign('level', $this->_level);
        View::assign('model', $this->_model);
        View::assign('tree', $this->_data);
        View::assign('page',$this->_page);
        View::assign('canMove',$this->_move);
        View::assign('canMerge',$this->_merge);
    ...
    public function doSetStatus($model, $ids, $status)
    {
        ...
        $this->success('Settings Saved', $_SERVER['HTTP_REFERER']);
    }
    ...
    /**
     * 设置新增页面方法名(该方式更容易让User立即，而setModel较容易在使用中产生歧义，setModel后期会废弃)
     *
