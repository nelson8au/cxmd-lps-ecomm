...
class AdminListBuilder extends AdminBuilder
{
     ...
     */
    public function buttonNew($href, $title = 'Add', $attr = [])
    {
        $attr['href'] = $href;
    ...
    public function buttonDisable($url = null, $title = 'Disable', $attr = [])
    {
    ...
    public function buttonEnable($url = null, $title = 'Enable', $attr = [])
    {
        if (!$url) $url = $this->_setStatusUrl;
     ...
     * 删除到回收站
     */
    public function buttonDelete($url = null, $title = 'Delete', $attr = [])
    {
        if (!$url) $url = $this->_setStatusUrl;
        $attr['class'] = 'btn btn-danger ajax-post confirm';
        $attr['data-confirm'] = 'Confirm Deletion';
        return $this->buttonSetStatus($url, -1, $title, $attr);
    }
     ...
     * 还原
     */
    public function buttonRestore($url = null, $title = 'Restore', $attr = [])
    {
        if (!$url) $url = $this->_setStatusUrl;
        return $this->buttonSetStatus($url, 1, $title, $attr);
    }
    ...
    {
        return $this->button(lang('Clear out'), ['class' => 'btn btn-danger ajax-post tox-confirm', 'data-confirm' => lang('Confirm clear out'), 'url' => url('', ['model' => $model]), 'target-form' => 'ids', 'hide-data' => 'true']);
    }
    ...
    public function buttonDeleteTrue($url = null)
    {
        ...
        $attr['data-confirm'] = lang('Confirm delete completely');
        $attr['url'] = $url;
        $attr['target-form'] = 'ids';
        return $this->button(lang('Delete completely'), $attr);
    }

    /**
     * 排序按钮
     */
    public function buttonSort($href, $title = 'Sort', $attr = [])
    {
        $attr['href'] = $href;
     ...
     * @param string $arrvalue 筛选数据（包含ID 和value的数组:array(array('id'=>1,'value'=>'System'),array('id'=>2,'value'=>'项目'),array('id'=>3,'value'=>'机构'));）
     * @return $this
     */
    public function search($title = 'Search', $name = 'key', $type = 'text', $des = '', $attr = '', $arrdb = '', $arrvalue = null)
    {
...
//            $this->setSearchPostUrl('');
     ...
     * @return $this
     */
    public function select($title = 'Filter', $name = 'key', $type = 'select', $des = '', $attr = '', $arrdb = '', $arrvalue = null)
    {
        if (empty($arrdb)) {
     ...
     * @return $this
     */
    public function keyIcon($name = 'icon', $title = 'Icon')
    {
    ...
    public function keyStatus($name = 'status', $title = 'Status')
    {
        $map = [-1 => 'Delete', 0 => 'Disable', 1 => 'Enable', -2 => 'Not Reviewed'];
    ...
    public function keyCreateTime($name = 'create_time', $title = 'Create Time')
    {
        return $this->keyTime($name, $title);
    }

    public function keyUpdateTime($name = 'update_time', $title = 'Update Time')
    {
        return $this->keyTime($name, $title);
    }

    public function keyUid($name = 'uid', $title = 'User')
    {
    ...
    public function keyDoAction($getUrl, $text, $title = 'Action', $class = 'btn-primary', $hide=[], $flag = 'id')
    {
        //获取默认getUrl函数
        if (is_string($getUrl)) {
            $getUrl = $this->createDefaultGetUrlFunction($getUrl,$flag);
    ...
    {
        return $this->keyDoAction($getUrl, $text, 'Action', 'ajax-get '. $class, $hide);
    }
    /**
     * 编辑操作
     * @param  [type] $getUrl [description]
     ...
     * @return [type]         [description]
     */
    public function keyDoActionEdit($getUrl, $text = 'Edit',$hide=[])
    {
        return $this->keyDoAction($getUrl, '<i class="icon icon-edit"></i> '.$text, 'Action', 'btn-success', $hide);
    }
    /**
     * Disable操作
     * @param  [type] $getUrl [description]
     ...
     * @return [type]         [description]
     */
    public function keyDoActionDisable($getUrl, $text = 'Disable', $hide=[])
    {
        return $this->keyDoAction($getUrl, '<i class="icon icon-minus-sign"></i> '. $text, 'Disable', 'btn-warning ajax-get', $hide);
    }
    /**
     * 删除操作
     * @param  [type] $getUrl [description]
     ...
     * @return [type]         [description]
     */
    public function keyDoActionDelete($getUrl, $text = 'Delete', $hide=[])
    {
        return $this->keyDoAction($getUrl, '<i class="icon icon-trash"></i> '.$text, 'Action','btn-danger ajax-get confirm', $hide);
    }
    /**
     * 还原操作，存在获取数据ID BUG
     * @param  string $text [description]
     * @return [type]       [description]
     */
    public function keyDoActionRestore($text = 'Restore', $hide=[])
    {
        $that = $this;
        ...
        return $this->keyDoAction($getUrl, $text,'Action','btn-primary ajax-get',$hide);
    }
    ...
    public function display($templateFile = '', $charset = '', $contentType = '', $content = '', $prefix = '', $solist = '')
    {
        ...
        View::assign('explain', $this->_explain);
        View::assign('list', $this->_data);
        /*加入搜索*/
        View::assign('searches', $this->_search);
        View::assign('searchPostUrl', $this->_searchPostUrl);
        /*加入筛选select*/
        View::assign('selects', $this->_select);
        View::assign('selectPostUrl', $this->_selectPostUrl);
     ...
     * 设置状态
     */
    public function doSetStatus($table, $ids, $status = 1)
    {
        $ids = array_unique((array)$ids);
        $ids = implode(',',$ids);
        $res = Db::name($table)->where('id','in', $ids)->update(['status' => $status]);
        if ($res) {
            return $this->success('Settings Saved', $res, $_SERVER['HTTP_REFERER']); 
        }else{
